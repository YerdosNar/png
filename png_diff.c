...
int main(int argc, char **argv) {
    ...
    if(idat_data && idat_size > 0) {
        ...
        if(image) {
            if(force_grayscale) {
                // Convert to grayscale and apply filter if needed
                ...
                if(image->channels >= 3) {
                    ...
                    // Save as RGB
                    uint8_t color_type = (image->channels == 4) ? 6 : 2; // 6 for RGBA, 2 for RGB
                    save_png(output_file, processed, image->width, image->height, color_type, image->channels);
                    free_pixel_matrix(processed, image->height);
                }
                else {
                    // Grayscale image with kernel
                    uint8_t **processed = allocate_pixel_matrix(image->height, image->width);
                ...
                // No filter, no grayscale conversion - just save as-is
                if(image->channels >= 3) {
                    // Save RGB/RGBA image
                    uint8_t color_type = (image->channels == 4) ? 6 : 2; // 6 for RGBA, 2 for RGB
...
// Replace the image processing section (starting from line 634) with:
